"""
Meeting Notes Graph (v1) — CocoIndex pipeline example, Apache AGE flavor.
...
entity resolution, and build a knowledge graph in Apache AGE:
...
Side-by-side diff with examples/meeting_notes_graph_neo4j/main.py: the
flow is identical; the only differences are the connector import, the
ConnectionFactory arguments (uri + auth + database vs host + port + graph),
and the AppConfig name.
"""
...
from cocoindex.connectors import age, google_drive
...
KG_DB = coco.ContextKey[age.ConnectionFactory]("kg_db")
...
async def coco_lifespan(
    ...
    builder.provide(
        ...
        age.ConnectionFactory(
            ...
            port=int(os.environ.get("AGE_PORT", "5455")),
            dbname=os.environ.get("AGE_DBNAME", "postgres"),
            user=os.environ.get("AGE_USER", "postgres"),
            password=os.environ.get("AGE_PASSWORD", "postgres"),
            graph=os.environ.get("AGE_GRAPH", "meeting_notes"),
        ),
    )
...
# ---------------------------------------------------------------------------
# AGE row schemas (dataclasses for declare_record / declare_relation)
# ---------------------------------------------------------------------------
...
async def process_file(
    ...
    meeting_table: age.TableTarget[Meeting],
    task_table: age.TableTarget[Task],
    decided_rel: age.RelationTarget[Any],
...
async def create_person_relations(
    ...
    person_table: age.TableTarget[Person],
    attended_rel: age.RelationTarget[Any],
    assigned_rel: age.RelationTarget[Any],
) -> None:
...
async def app_main() -> None:
    meeting_table = await age.mount_table_target(
        KG_DB,
        "Meeting",
        await age.TableSchema.from_class(Meeting, primary_key="id"),
        primary_key="id",
    )
    person_table = await age.mount_table_target(
        KG_DB,
        "Person",
        await age.TableSchema.from_class(Person, primary_key="name"),
        primary_key="name",
    )
    task_table = await age.mount_table_target(
        KG_DB,
        "Task",
        await age.TableSchema.from_class(Task, primary_key="description"),
        primary_key="description",
    )

    attended_rel = await age.mount_relation_target(
        KG_DB, "ATTENDED", person_table, meeting_table
    )
    decided_rel = await age.mount_relation_target(
        KG_DB, "DECIDED", meeting_table, task_table
    )
    assigned_rel = await age.mount_relation_target(
...
app = coco.App(
    coco.AppConfig(name="MeetingNotesGraphAGE"),
    app_main,
)
